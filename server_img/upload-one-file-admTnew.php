...
$address_site = '/adm/content/tovar.php'; // Адрес сайта
...
$log = $connection->real_escape_string($_POST["name"]);
$sname = $connection->real_escape_string($_POST["price"]);

$color = $connection->real_escape_string($_POST["color"]);
if ($color !== '0') {
    $col = $connection->real_escape_string($_POST["color"]);
} else {
    $col = $product['id_cr'];
}

$patrs = $connection->real_escape_string($_POST["gen"]);
if ($patrs !== '0') {
    $patr = $connection->real_escape_string($_POST["gen"]);
} else {
    $patr = $product['id_gr'];
}

$categ = $connection->real_escape_string($_POST["cat"]);
if ($categ !== '0') {
...
$name = $connection->real_escape_string($_POST["countries"]);


if (isset($_POST['upload_images'])) {


    $sql = "INSERT INTO `product` (`name`, `price`, `countries`, `category`, `gender`, `color`, `img`) VALUES('$log', '$sname', '$name', '$cat', '$patr','$col', '$img')";
...
$allow_types = ['image/jpeg', 'image/png', 'image/jpg', 'image/jfif'];
...
$allow_extensions = ['jpg', 'jpeg', 'png', 'jfif'];
...
$result_move = move_uploaded_file($_FILES['file_img']['tmp_name'], '../images/special/' . $file_name . '.' . $extension_file);
