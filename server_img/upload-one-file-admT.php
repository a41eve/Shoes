...
$product = mysqli_query($connection, "SELECT * FROM `product`, `gender` WHERE `id_pt` = '$polz' AND `product`.`gender` = `gender`.`id_gr`" );
 ...
 $log = $connection->real_escape_string($_POST["name"]);
  ...
  if($patrs !== '0'){
   $patr = $connection->real_escape_string($_POST["gen"]);
    ...
    $sql = "UPDATE `product` SET `name`='$log', `price`='$sname', `countries`='$name', `gender`='$patr', `img`='$img' WHERE `id_pt` = '$polz'"; 
