...
$product = mysqli_query($connection, "SELECT * FROM `info`");
$product = mysqli_fetch_assoc($product);

session_start();

$protocol = isset($_SERVER['HTTPS']) ? 'https://' : 'http://';
$address_site = '/adm/content/info.php'; // Адрес сайта
...
$slog = $connection->real_escape_string($_POST["slog"]);
$desk = $connection->real_escape_string($_POST["desk"]);
$addr = $connection->real_escape_string($_POST["addr"]);
...
if (isset($_POST['upload_image'])) {
    ...
    $sql = "UPDATE `info` SET `slog`='$slog', `img`='$img', `desk`='$desk', `addr`='$addr', `tel`='$tel', `email`='$email'";

    if ($connection->query($sql)) {
        header('Location: /adm/content/info.php');
    } else {
